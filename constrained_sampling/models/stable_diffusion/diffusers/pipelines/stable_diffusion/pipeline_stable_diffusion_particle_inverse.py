...
class StableDiffusionParticleInversePipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def sample(
        ...
        # Create optimizers
        latents = torch.autograd.Variable(latents, 
                                          requires_grad=True).cuda() 
        optimizer_z = torch.optim.Adam([latents], lr=lr_z, betas=(0.9, 0.99), weight_decay=0.0)   #original: 0.999

        x_init = torch.randn((batch_size * num_images_per_prompt, 3, height, width)).cuda()
        x_pred = torch.autograd.Variable(x_init,
                                        requires_grad=True).cuda() 
        optimizer_x = torch.optim.Adam([x_pred], lr=lr_x, betas=(0.9, 0.99), weight_decay=0.0)   #original: 0.999

        # Create this directory /home/nzilberstein/Inverse/_exp/evol if it doesn't exist, and it exists, rewrite
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in tqdm(enumerate(timesteps[:-1])):
                ...
                latent_model_input = torch.cat([latent_pred_t] * 2) if do_classifier_free_guidance else latent_pred_t
                ...
                # noise_pred_uncond, noise_pred_text = et.chunk(2)
                ...
                if dino is not None:
                    ...
                    x_pred_z = self.decode_latents(latent_pred_t, stay_on_device=True)
                    ...
                    # et = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                ...
                ###
                # Case without augmentation
                ###
                # compute the error between the x_pred and the x_pred_z after decoding

                # x_pred_z = self.decode_latents(latents, stay_on_device=True)
                # e_obs = y_0 - H.H(x_pred_z)
                # loss_obs = (e_obs**2).mean()/2

                # loss_diffusion = torch.mul((noise_pred).detach(), latents).mean()

                # loss_z = loss_obs + w_t * snr_inv * loss_diffusion
                ...
                # ---------------------------- #
                ###
                # Case with augmentation
                ###               
                ## Optimize z ##
                x_pred_z = self.decode_latents(latents, stay_on_device=True)
                e_decod = x_pred - x_pred_z
                loss_reg = (e_decod**2).mean() / 2

                loss_diffusion = torch.mul((noise_pred).detach(), latents).mean()

                # Compute loss
                # rho_reg_ = rho_reg * (1-alpha_prod_t).sqrt().detach()

                loss_z = rho_reg * loss_reg + w_t * snr_inv * loss_diffusion
                ...
                ## Optimize x ##
                x_pred_z = self.decode_latents(latents , stay_on_device=True).detach()
                e_decod = x_pred - x_pred_z
                loss_reg = (e_decod**2).mean() / 2

                e_obs = y_0 - H.H(x_pred)
                loss_obs = (e_obs**2).mean() / 2
                ...
                # Save images in list
                image_list.append(x_pred)

                # Save image every 100 steps
                if counter % 100 == 0:
                    image_evol = make_grid(postprocess(x_pred.clone().detach().cpu()))
        ...
        return StableDiffusionPipelineOutput(images=x_pred, nsfw_content_detected=has_nsfw_concept)
