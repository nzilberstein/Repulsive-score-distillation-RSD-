...
def build_one_degredation_model(cfg, h, w, c, deg: str):
    ...
    if deg == "deno":
        ...
        if deg == 'inp_lolcat':
            ...
            masks_root = os.path.join(exp_root, 'masks', f'{deg_type}_extra.npy')
            loaded = np.load(masks_root)
            mask = torch.from_numpy(loaded).cuda().reshape(-1)
            missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
        elif deg == 'inp_lorem':
            exp_root = cfg.exp.root
            deg_type = deg.split('_')[-1]
            masks_root = os.path.join(exp_root, 'masks', f'{deg_type}3.npy')
            loaded = np.load(masks_root)
            mask = torch.from_numpy(loaded).cuda().reshape(-1)
            missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
        elif deg == 'inp_20ff':
            ...
            if w == 256:
                ...
                mask = torch.from_numpy(loaded).cuda()
                mask = mask.long()
                mask = F.interpolate(mask.unsqueeze(0).unsqueeze(0).float(), scale_factor=2, mode='nearest').squeeze()
                mask = (mask == 1).reshape(-1)
            # mask = torch.repeat_interleave(mask, 2) 
            # mask = mask.repeat(4)
            missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
            ...
            # missing_r = torch.randperm(w**2)[:w**2 // 2].cuda().long() * 3
            
            # missing_r = torch.randperm(w**2)[:(w-5)**2].cuda().long() * 3
            # missing_r = torch.arange(0, w**2)[w**2 // 2 - w**2 // 4:w**2 // 2 + w**2 // 4].cuda().long() * 3
        elif deg =='inp_random':
        ...
        H = Inpainting(c, w, missing, device)
    # elif deg[:3] == "inp":
    #     exp_root = cfg.exp.root
    #     deg_type = deg.split('_')[-1]
    #     masks_root = os.path.join(exp_root, 'masks', f'{deg_type}.npz')
