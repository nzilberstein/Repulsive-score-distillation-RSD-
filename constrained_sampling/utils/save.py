...
def save_imagenet_result_particles(x, y, info, samples_root, cfg, suffix=""):
        

    # x = x.view(-1, cfg.algo.n_particles, cfg.dataset.n_channels, cfg.dataset.image_size, cfg.dataset.image_size)
    n = x.size(0)
    ...
    for i in range(n):
        ...
        os.makedirs(class_dir, exist_ok=True)
    for i in range(n):
        for j in range(cfg.algo.n_particles):
            if len(suffix) > 0:
                tvu.save_image(x[i][j], os.path.join(samples_root, info["class_id"][i], f'{info["name"][i]}_{suffix}_{j}.png'))
            else:
                tvu.save_image(x[i][j], os.path.join(samples_root, info["class_id"][i], f'{info["name"][i]}_{j}.png'))
...
def save_ffhq_result(x, y, info, samples_root, suffix=""):
    ...
    output_path = os.path.join(samples_root, f'{idx[0].split('.')[0]}')
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    n = x.shape[0]
    for i in range(n):
        tvu.save_image(x[i], os.path.join(output_path, f'x_{i}.png'))
