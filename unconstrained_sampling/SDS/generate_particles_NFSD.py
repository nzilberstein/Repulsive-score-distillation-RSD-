...
from diffusers import StableDiffusionParticleNFSDPipeline, DDIMScheduler, EulerDiscreteScheduler
import torch
...
parser.add_argument('--save_path', type=str, default='/home/nzilberstein/red_diff_stable/RED-diff_stable/particle_guidance/stable_diffusion/generated_images_NFSD')
...
for cnt, mini_batch in enumerate(tqdm.tqdm(all_text)):
    ...
    neg_prompt = neg_prompt * args.n_particles
    # print(text)
    if args.dino:
        out = pipe.dino_NFSD(text, generator=generator, num_inference_steps=args.steps, coeff= args.coeff, guidance_scale=args.w, dino=dino, lr = args.lr, output_type='tensor', negative_prompt=neg_prompt, t_repul=args.t_repul)
    else:
