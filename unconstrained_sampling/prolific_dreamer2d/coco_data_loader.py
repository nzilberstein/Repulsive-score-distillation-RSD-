...
def print_folder_names(directory_path):
    # Get the list of all items (files and folders) in the specified directory
...
class text_image_pair(torch.utils.data.Dataset):
    def __init__(self, dir_path, csv_path, prompt=None, group=False, only_text=False):
        ...
        self.img_dir_list = print_folder_names(dir_path)
    ...
    def __getitem__(self, idx):
        ...
        if self.group:
            path = self.img_dir_list[idx]
            img_dir = os.path.join(self.dir_path, f'{path}')
