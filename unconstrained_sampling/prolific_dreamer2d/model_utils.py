...
def sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings, t, unet_phi=None, guidance_scale=7.5, \
                        grad_scale=1, cfg_phi=1., generation_mode='sds', phi_model='lora', \
                            cross_attention_kwargs={}, multisteps=1, scheduler=None, lora_v=False, \
                                half_inference = False, dino = None, vae=None,
                                sigma_t = None, coeff = 40., lpips=None):
    ...
    if generation_mode == 'sds':
        ...
        if dino is not None:
            ...
            # x_pred = (noisy_latents - sigma_t * noise_pred) / (1 - sigma_t)
            x_pred = vae.decode(noisy_latents * 1 / vae.config.scaling_factor).sample
            # print(x_pred.shape)
            ...
            deps_dx_backprop = torch.autograd.grad(eval_sum, noisy_latents)[0]
            grad_phi = deps_dx_backprop.view_as(noisy_latents)

            
            K_svgd_z_mat_reg_sum = weights.sum(dim = 1)
            nabla_log = torch.div(grad_phi, K_svgd_z_mat_reg_sum.unsqueeze(-1).unsqueeze(-1))
            # print(coeff)
            grad = grad_scale * (noise_pred - noise_pred_phi.detach() - coeff * sigma_t * nabla_log.detach())
        else:
