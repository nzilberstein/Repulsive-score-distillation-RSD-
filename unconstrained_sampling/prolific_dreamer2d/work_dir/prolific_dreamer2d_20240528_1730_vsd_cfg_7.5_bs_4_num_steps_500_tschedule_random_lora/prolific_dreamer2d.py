...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--dino_flag', action='store_true', default=False, help='use dino')
    parser.add_argument('--lpips_flag', action='store_true', default=False, help='use dino')
    parser.add_argument('--coeff', type=float, default=0., help='weight of repulsion')
    parser.add_argument('--gpu', type=int, default=0, help='gpu id')
    parser.add_argument('--index_folder', type=int, default=0, help='gpu id')
    parser.add_argument('--save_path', type=str, default='/home/nzilberstein/repository/unconstrained_sampling/prolific_dreamer2d/generated_images', help='save path')
...
def main():
    ...
    if args.dino_flag == True:
        dino = torch.hub.load('facebookresearch/dino:main', 'dino_vits16').to("cuda")
        lpips_ = lpips.LPIPS(net='alex').cuda()
        logger.info('Dino loaded, and dino flag is true')
    elif args.lpips_flag == True:
        lpips_ = lpips.LPIPS(net='alex').cuda()
        logger.info('lpips loaded, and lpips flag is true')
    else:
        dino = None
        logger.info('Non repulsion')
    #######################################################################################
    ### load model
    ...
    if args.generation_mode == 'vsd':
        if args.phi_model in ['lora', 'unet_simple']:
            ...
            # phi_optimizer = torch.optim.Adam(phi_params, lr=args.phi_lr, betas=(0.9, 0.99), weight_decay=0.0)
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            ...
            # loss step
            if args.dino_flag is not False:
                logger.info("DINO mode")
                grad_, noise_pred, noise_pred_phi = sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings_vsd, t, \
                                                    ...
                                                    cfg_phi=args.cfg_phi, grad_scale=args.grad_scale, dino = dino, vae = vae, sigma_t = loss_weights[int(t)].sqrt(), coeff=args.coeff)                
            else:
                logger.info("Non repulsion mode")
                grad_, noise_pred, noise_pred_phi = sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings_vsd, t, \
                                                    guidance_scale=args.guidance_scale, unet_phi=unet_phi, \
                                                        generation_mode=args.generation_mode, phi_model=args.phi_model, \
                                                            cross_attention_kwargs=cross_attention_kwargs, \
                                                                multisteps=args.multisteps, scheduler=scheduler, lora_v=args.lora_vprediction, \
                                                                    half_inference=args.half_inference, \
                                                                        cfg_phi=args.cfg_phi, grad_scale=args.grad_scale, coeff=args.coeff)
            ## weighting
    ...
    # Save images to compare with SDS and repulsion
    dir_dino = os.path.join(f'{args.save_path}', str(args.coeff), str(args.index_folder - 2))
    if os.path.exists(dir_dino):
        shutil.rmtree(dir_dino)
    os.makedirs(dir_dino)
    for i in range(args.batch_size):
        save_image((image[i]/2+0.5).clamp(0, 1), os.path.join(dir_dino, f'image_{i}.png'))
        
    dino = torch.hub.load('facebookresearch/dino:main', 'dino_vits16').to("cuda")
    total_pair_wise_sim = 0.
    xo_list = dino((image/2+0.5).clamp(0, 1).cuda())
    # del dino
    ...
    save_image((images/2+0.5).clamp(0, 1), f'{args.work_dir}/final_image_2_{image_name}.pdf')
