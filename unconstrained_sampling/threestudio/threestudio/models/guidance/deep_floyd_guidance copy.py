...
class DeepFloydGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        self.dino = torch.hub.load('facebookresearch/dino:main', 'dino_vits16').to("cuda")
        self.lpips_ = lpips.LPIPS(net='alex').cuda() # best forward score
        self.epoch = 0
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if t < 200:
            ...
            noise_pred = (noise_pred_uncond - noise_pred_neg) + self.cfg.guidance_scale * (noise_pred_text - noise_pred_uncond) 
...
#####################
        ...
        # x_pred = self.decode_latents(latents_noisy, stay_on_device=True)
        dino_out = self.dino(latents_noisy)
        ...
        deps_dx_backprop = torch.autograd.grad(eval_sum, latents_noisy)[0]
        ...
        # print(nabla_log.shape)

        # latents_noisy.requires_grad_(True)
        # self.lpips_.requires_grad_(True)
        # self.lpips_.train()

        # x1 = latents_noisy.repeat(latents_noisy.size(0), 1, 1, 1)
        # x2 = torch.repeat_interleave(latents_noisy, latents_noisy.size(0), dim=0)

        # # N x N x 1
        # distance = self.lpips_(x1, x2).view(latents_noisy.size(0), latents_noisy.size(0), 1)
        # num_images = latents_noisy.shape[0]
        # h_t = (distance.median(dim=1, keepdim=True)[0]) ** 2 / np.log(
        #     num_images - 1)
        # weights = torch.exp(- (distance ** 2 / h_t))

        # grad_phi = torch.autograd.grad(torch.sum(distance), latents_noisy)[0]
        # grad_phi = 2 * torch.einsum('bik,bkj->bj', weights.view(1, weights.size(0), weights.size(1)),
        #                             grad_phi.view(1, latents_noisy.size(0), -1)) / h_t
        # grad_phi = torch.squeeze(grad_phi, dim=1)
        # grad_phi = grad_phi.view(latents_noisy.size(0), latents_noisy.size(1), latents_noisy.size(2), latents_noisy.size(3))
        # K_svgd_z_mat_reg_sum = weights.sum(dim = 1)
        # nabla_log = torch.div(grad_phi, K_svgd_z_mat_reg_sum.unsqueeze(-1).unsqueeze(-1))
######################
        # # 40, 10
        if self.epoch < 2000:
            grad = w * (noise_pred - noise - 0 * (1 - self.alphas[t]).sqrt().view(-1, 1, 1, 1) * nabla_log)
        else:
            grad = w * (noise_pred - noise - 0 * (1 - self.alphas[t]).sqrt().view(-1, 1, 1, 1) * nabla_log)
