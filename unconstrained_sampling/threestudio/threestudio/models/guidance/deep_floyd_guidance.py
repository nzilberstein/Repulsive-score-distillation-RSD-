...
class DeepFloydGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        self.dino_flag = False
        self.gamma = 200 #100
        print(self.gamma, self.dino_flag)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.cfg.weighting_strategy == "sds":
            ...
            # w = ((1 - self.alphas[t]).sqrt() / (self.alphas[t]).sqrt()).view(-1, 1, 1, 1)
...
#####################
        ...
        if self.dino_flag == True:
            ...
            # - (1 - self.alphas[t]).sqrt().view(-1, 1, 1, 1) * 
    ...
    ######################
            # # 40, 10
            grad = w * (noise_pred - noise -  self.gamma * (1 - self.alphas[t]).view(-1, 1, 1, 1) * nabla_log)
            # print(self.gamma * (1 - self.alphas[t]).sqrt().view(-1, 1, 1, 1) * nabla_log)
        else:
