...
class StableDiffusionGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        # print(self.dino.dino_flag, self.dino.gamma)
    ...
    def compute_grad_sds(
        ...
        if self.dino_flag == True:
            ...
            x_pred = self.decode_latents(latents_noisy)
            # - (1 - self.alphas[t]).sqrt().view(-1, 1, 1, 1) * 
            dino_out = self.dino(x_pred)
            ...
            grad = w * (noise_pred - noise +  self.gamma * (1 - self.alphas[t]).pow(1/2).view(-1, 1, 1, 1) * grad_phi)
            print(t, self.cfg.guidance_scale)
            # print( (noise_pred - noise)[:,:,0:5,0:5], self.gamma * (1 - self.alphas[t]).pow(1/8).view(-1, 1, 1, 1) * nabla_log[:,:,0:5,0:5])
        else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(t, self.min_step, self.max_step + 1)
